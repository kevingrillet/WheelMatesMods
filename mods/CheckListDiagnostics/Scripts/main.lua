...
local function log(message) print(string.format("[%s] %s\n", ModName, message)) end

--- Returns true only for a reflected Unreal object that can still be inspected.
local function is_valid(object) return object ~= nil and object.IsValid ~= nil and object:IsValid() end
...
local function for_each_object(objects, callback)
    if type(objects) == "table" then for _, object in pairs(objects) do callback(unwrap(object)) end
    elseif objects ~= nil and objects.ForEach ~= nil then objects:ForEach(function(_, object) callback(unwrap(object)) end) end
end
...
local function report_discovery()
    ...
    for_each_object(FindAllOf("Actor"), function(actor)
        ...
        if string.find(short_name, "collectable", 1, true) or string.find(short_name, "collectible", 1, true) or string.find(short_name, "scannable", 1, true) or string.find(short_name, "neuro", 1, true) then
    ...
    local classes = {}
    for class in pairs(matches) do table.insert(classes, class) end
    table.sort(classes)
    log("Collectible actor discovery (read-only)")
    for _, class in ipairs(classes) do
        local entry, properties = matches[class], {}
        entry.actor:GetClass():ForEachProperty(function(property) table.insert(properties, property:GetFName():ToString()) end)
...
local function report_gears_and_unlocks()
    ...
    for_each_object(FindAllOf("Actor"), function(actor)
        if is_valid(actor) and string.find(string.lower(actor:GetFullName()), "bp_collectable_gear", 1, true) then
            ...
            count = count + 1
            log(string.format("Gear candidate | %s | {X=%.3f, Y=%.3f, Z=%.3f}", actor:GetFullName(), location.X, location.Y, location.Z))
        end
    end)
    ...
    for_each_object(FindAllOf("VehicleSaveGame"), function(save)
        ...
        for tag in pairs(current) do
            total = total + 1
            if previous_customization_tags ~= nil and not previous_customization_tags[tag] then log("New customization unlock | " .. tag); added = added + 1 end
        end
        log(string.format("Customization unlock tags saved: %d.", total))
        if previous_customization_tags == nil then log("Customization snapshot saved; probe again after collecting a Gear to compare.")
        elseif added == 0 then log("No new customization tag since the previous Gear probe.") end
        previous_customization_tags = current
    end)
    log("Gear and customization probe complete.")
end

RegisterKeyBind(Key.NUM_ONE, { ModifierKey.CONTROL }, function() ExecuteInGameThread(report_discovery) end)
RegisterKeyBind(Key.NUM_TWO, { ModifierKey.CONTROL }, function() ExecuteInGameThread(report_gears_and_unlocks) end)
