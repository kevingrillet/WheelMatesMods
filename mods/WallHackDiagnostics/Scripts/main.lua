...
local ModName = "WallHackDiagnostics"
...
local function local_player_pawns()
    ...
    if not is_valid(game_instance) then return pawns end

    local players = game_instance.LocalPlayers
    if players == nil then return pawns end
...
local function report_player_outline_state()
    ...
    for _, player in ipairs(players) do
        ...
        for_each_object(components, function(component)
            ...
            if string.find(component:GetFullName(), pawn_path, 1, true) == nil then return end
            if string.find(class_name(component), "StaticMeshComponent", 1, true) == nil then return end
            matched = matched + 1
            local depth_ok, depth = pcall(function() return component.bRenderCustomDepth end)
            local stencil_ok, stencil = pcall(function() return component.CustomDepthStencilValue end)
            log(string.format(
...
local function report_gear_render_components()
    ...
    for_each_object(FindAllOf("BP_Collectable_Gear_C"), function(gear)
        ...
        for_each_object(components, function(component)
            if not is_valid(component) then return end
            if string.find(component:GetFullName(), gear_path, 1, true) == nil then return end
            if string.find(class_name(component), "StaticMeshComponent", 1, true) == nil then return end
...
RegisterKeyBind(Key.F8, { ModifierKey.CONTROL }, function() ExecuteInGameThread(report_player_outline_state) end)
RegisterKeyBind(Key.F4, { ModifierKey.CONTROL, ModifierKey.SHIFT }, function() ExecuteInGameThread(report_gear_render_components) end)
